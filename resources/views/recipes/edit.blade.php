<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('recipes.update', $recipe) }}" enctype="multipart/form-data"
              x-data="recipeForm({{ json_encode($recipe->ingredients->map(fn($i) => ['amount' => $i->amount, 'unit' => $i->unit, 'name' => $i->name, 'group' => $i->group, 'known_ingredient_id' => $i->known_ingredient_id ?? ''])) }}, {{ json_encode($recipe->steps->map(fn($s) => ['instruction' => $s->instruction])) }})"
              class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2" maxlength="300"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500">{{ old('description', $recipe->description) }}</textarea>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                    ...
                    <x-text-input id="prep_time_minutes" name="prep_time_minutes" type="number" min="0" class="mt-1 block w-full" :value="old('prep_time_minutes', $recipe->prep_time_minutes)" />
                </div>
                <div>
                    <x-input-label for="cook_time_minutes" value="Cook (min)" />
                    <x-text-input id="cook_time_minutes" name="cook_time_minutes" type="number" min="0" class="mt-1 block w-full" :value="old('cook_time_minutes', $recipe->cook_time_minutes)" />
                </div>
                <div>
                    <x-input-label for="servings" value="Servings" />
                    <x-text-input id="servings" name="servings" type="number" min="1" class="mt-1 block w-full" :value="old('servings', $recipe->servings)" required />
                </div>
                <div>
                    ...
                    <select id="difficulty" name="difficulty" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500">
                        @foreach(['easy', 'medium', 'hard'] as $d)
                            <option value="{{ $d }}" {{ old('difficulty', $recipe->difficulty) === $d ? 'selected' : '' }}>{{ ucfirst($d) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="rest_time_minutes" name="rest_time_minutes" type="number" min="0" class="mt-1 block w-full" :value="old('rest_time_minutes', $recipe->rest_time_minutes)" />
                </div>
                <div>
                    <x-input-label for="yield" value="Yield (optional)" />
                    <x-text-input id="yield" name="yield" type="text" class="mt-1 block w-full" :value="old('yield', $recipe->yield)" />
                </div>
            </div>

            <!-- Image -->
            <div>
                <x-input-label for="image" value="Photo (leave empty to keep current)" />
                @if($recipe->image_path)
                    <div class="mt-2 mb-2">
                        <img src="{{ asset('storage/' . $recipe->image_path) }}" alt="Current photo" class="h-32 rounded-lg object-cover">
                    </div>
                @endif
            ...
            <div>
                ...
                <x-text-input id="tags" name="tags" type="text" class="mt-1 block w-full"
                    :value="old('tags', $recipe->tags->pluck('name')->implode(', '))" placeholder="e.g. vegetarian, quick, summer" />
            </div>
            ...
            <div>
                    ...
                    <template x-for="(ingredient, index) in ingredients" :key="index">
                        <div class="flex gap-2 items-start">
                            ...
                            <select x-bind:name="'ingredients[' + index + '][unit]'" x-model="ingredient.unit"
                                ...
                                <option value="">—</option>
                                <optgroup label="Metric"><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option></optgroup>
                                <optgroup label="Imperial"><option value="cup">cup</option><option value="tbsp">tbsp</option><option value="tsp">tsp</option><option value="oz">oz</option><option value="lb">lb</option><option value="fl oz">fl oz</option></optgroup>
                                <optgroup label="Other"><option value="pinch">pinch</option><option value="piece">piece</option><option value="slice">slice</option><option value="clove">clove</option><option value="bunch">bunch</option><option value="can">can</option><option value="whole">whole</option></optgroup>
                            </select>
                ...
                <button type="button" @click="addIngredient()" class="mt-3 text-sm text-orange-600 hover:text-orange-700 font-medium">+ Add Ingredient</button>
            </div>
            ...
            <div>
                ...
                <div class="space-y-3 mt-2">
                    <template x-for="(step, index) in steps" :key="index">
                        <div class="flex gap-3 items-start">
                            <div class="flex-shrink-0 w-8 h-8 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-sm mt-1" x-text="index + 1"></div>
                ...
                <button type="button" @click="addStep()" class="mt-3 text-sm text-orange-600 hover:text-orange-700 font-medium">+ Add Step</button>
            </div>
            ...
            <div class="flex gap-4 pt-4 border-t border-gray-200">
                <button type="submit" name="status" value="published"
                    ...
                    {{ $recipe->status === 'published' ? 'Update Recipe' : 'Publish Recipe' }}
                </button>
